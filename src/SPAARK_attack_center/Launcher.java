                    ...
                    if (surroundingLaunchers >= 2) {
                        if (lowestIdFriendlyRobotInfo.ID > rc.getID()) {
                            if (opponentLocation != null) {
                                ...
                                Direction[] bug2array = Motion.bug2(rc, new MapLocation(rc.getMapWidth() - prioritizedHeadquarters.x, rc.getMapHeight() - prioritizedHeadquarters.y), lastDirection, clockwiseRotation, indicatorString);
                                lastDirection = bug2array[0];
                                if (bug2array[1] == Direction.CENTER) {
                                    clockwiseRotation = !clockwiseRotation;
                                }
