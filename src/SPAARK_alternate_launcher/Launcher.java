package SPAARK_alternate_launcher;
...
public strictfp class Launcher {
    ...
    private int turnsNotAttacked = 0;
    ...
    private void runState() throws GameActionException {
        ...
        if ((round + rc.getID()) % 2 == 0) {
            ...
            if (friendlyRobotInfo.length > 0) {
                ...
                RobotInfo closestCenterFriendlyRobotInfo = null;
                for (RobotInfo w : friendlyRobotInfo) {
                    if (w.getType() != RobotType.LAUNCHER) {
                        continue;
                    }
                    surroundingLaunchers += 1;
                    if (closestCenterFriendlyRobotInfo == null) {
                        closestCenterFriendlyRobotInfo = w;
                        continue;
                    }
                    if (closestCenterFriendlyRobotInfo.getLocation().distanceSquaredTo(prioritizedHeadquarters) - closestCenterFriendlyRobotInfo.getLocation().distanceSquaredTo(new MapLocation(rc.getMapWidth() / 2, rc.getMapHeight() / 2)) < w.getLocation().distanceSquaredTo(prioritizedHeadquarters) - w.getLocation().distanceSquaredTo(new MapLocation(rc.getMapWidth() / 2, rc.getMapHeight() / 2))) {
                        closestCenterFriendlyRobotInfo = w;
                    }
                }
                if (surroundingLaunchers >= 5) {
                    // try to swarm, lots of launchers, more aggressive
                    if (closestCenterFriendlyRobotInfo.getLocation().distanceSquaredTo(prioritizedHeadquarters) - closestCenterFriendlyRobotInfo.getLocation().distanceSquaredTo(new MapLocation(rc.getMapWidth() / 2, rc.getMapHeight() / 2)) < me.distanceSquaredTo(prioritizedHeadquarters) - me.distanceSquaredTo(new MapLocation(rc.getMapWidth() / 2, rc.getMapHeight() / 2))) {
                        // i'm the leader!
                                    ...
                                    if (prioritizedIslandLocation != null) {
                                        ...
                                        if (symmetry == 1) {
                                            ...
                                            if ((globalArray.mapSymmetry() & 0b1) == 1) {
                                                ...
                                                Direction[] bug2array = Motion.bug2(rc, new MapLocation(rc.getMapWidth() - 1 - targetHeadquarters.x, targetHeadquarters.y), lastDirection, clockwiseRotation, false, false, indicatorString);
                                                lastDirection = bug2array[0];
                                                if (bug2array[1] == Direction.CENTER) {
                                                    clockwiseRotation = !clockwiseRotation;
                                                }
                                                ...
                                                Direction[] bug2array = Motion.bug2(rc, new MapLocation(targetHeadquarters.x, rc.getMapHeight() - 1 - targetHeadquarters.y), lastDirection, clockwiseRotation, false, false, indicatorString);
                                                lastDirection = bug2array[0];
                                                if (bug2array[1] == Direction.CENTER) {
                                                    clockwiseRotation = !clockwiseRotation;
                                                }
                                                ...
                                                Direction[] bug2array = Motion.bug2(rc, new MapLocation(rc.getMapWidth() - 1 - targetHeadquarters.x, rc.getMapHeight() - 1 - targetHeadquarters.y), lastDirection, clockwiseRotation, false, false, indicatorString);
                                                lastDirection = bug2array[0];
                                                if (bug2array[1] == Direction.CENTER) {
                                                    clockwiseRotation = !clockwiseRotation;
                                                }
                                        ...
                                        // }
                                            ...
                                            for (int i = 0;i < wellLocations.length;i++) {
                                                ...
                                                else if (m.distanceSquaredTo(me) < prioritizedWellLocation.distanceSquaredTo(me)) {
                                                    prioritizedWellLocation = m;
                                                }
                                            }
                                            if (rc.getID() % 2 == 0) {
                                                if (me.distanceSquaredTo(prioritizedWellLocation) >= 10) {
                                                    Direction[] bug2array = Motion.bug2(rc, prioritizedWellLocation, lastDirection, clockwiseRotation, false, true, indicatorString);
                                                    lastDirection = bug2array[0];
                                                    if (bug2array[1] == Direction.CENTER) {
                                                        clockwiseRotation = !clockwiseRotation;
                                                    }
                                                    ...
                                                    clockwiseRotation = Motion.circleAroundTarget(rc, prioritizedWellLocation, 8, clockwiseRotation, true, true);
                                                }
                                            }
                                            else {
                        ...
                        if (opponent != null) {
                            ...
                            // else if (lastLauncherLocation != null && rc.canMove(lastLauncherLocation.directionTo(closestCenterFriendlyRobotInfo.getLocation()))) {
                            //     // follow leader last move
                            //     rc.move(lastLauncherLocation.directionTo(closestCenterFriendlyRobotInfo.getLocation()));
                            //     lastLauncherLocation = closestCenterFriendlyRobotInfo.getLocation();
                            // }
                            else {
                                if (me.distanceSquaredTo(closestCenterFriendlyRobotInfo.getLocation()) <= launcherCircleRange * 1.5) {
                                    if (lastLauncherLocation != null && rc.canMove(lastLauncherLocation.directionTo(closestCenterFriendlyRobotInfo.getLocation()))) {
                                        // follow leader last move
                                        rc.move(lastLauncherLocation.directionTo(closestCenterFriendlyRobotInfo.getLocation()));
                                        lastLauncherLocation = closestCenterFriendlyRobotInfo.getLocation();
                                    }
                                        ...
                                        clockwiseRotation = Motion.circleAroundTarget(rc, closestCenterFriendlyRobotInfo.getLocation(), launcherCircleRange, clockwiseRotation, true, true);
                                        lastLauncherLocation = closestCenterFriendlyRobotInfo.getLocation();
                                    }
                                }
                                else {
                                    // i can see leader but not close enough, try to bug to leader
                                    Direction[] bug2array = Motion.bug2(rc, closestCenterFriendlyRobotInfo.getLocation(), lastDirection, clockwiseRotation, true, true, indicatorString);
                                    lastDirection = bug2array[0];
                                    if (bug2array[1] == Direction.CENTER) {
                                        clockwiseRotation = !clockwiseRotation;
                                    }
                        ...
                        me = rc.getLocation();
                        if (GlobalArray.DEBUG_INFO >= 3) {
                            rc.setIndicatorLine(me, closestCenterFriendlyRobotInfo.getLocation(), 255, 255, 75);
                        }
                        else if (GlobalArray.DEBUG_INFO >= 2) {
                    ...
                    if (closestCenterFriendlyRobotInfo.getLocation().distanceSquaredTo(prioritizedHeadquarters) - closestCenterFriendlyRobotInfo.getLocation().distanceSquaredTo(new MapLocation(rc.getMapWidth() / 2, rc.getMapHeight() / 2)) < me.distanceSquaredTo(prioritizedHeadquarters) - me.distanceSquaredTo(new MapLocation(rc.getMapWidth() / 2, rc.getMapHeight() / 2))) {
                        ...
                        //     Direction direction = lastLauncherLocation.directionTo(closestCenterFriendlyRobotInfo.getLocation());
                        ...
                        //     }
                        // }
                        if (me.distanceSquaredTo(closestCenterFriendlyRobotInfo.getLocation()) <= launcherCircleRange * 1.25) {
                            // circle around leader
                            clockwiseRotation = Motion.circleAroundTarget(rc, closestCenterFriendlyRobotInfo.getLocation(), launcherCircleRange, clockwiseRotation, true, true);
                            lastLauncherLocation = closestCenterFriendlyRobotInfo.getLocation();
                        }
