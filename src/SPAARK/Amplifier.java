        ...
        while (true) {
            try {
                ...
                if (robot != null && Attack.prioritizedRobot(robot.getType()) >= 3) {
                    ...
                    state = 2;
                    noRobotTime = Math.max(noRobotTime - 5, 0);
                }
    ...
    private void runState() throws GameActionException {
        if (state == 0) {
            ...
            if (!isRandomExplorer) {
                    ...
                    if (noRobotTime >= 30) {
                        if (!storedLocations.arrivedAtWell) {
                            ...
                            if (headquarterID + 1 == headquarters.length) {
                                rc.writeSharedArray(GlobalArray.OPPONENT_HEADQUARTERS, 1 << 2);
                            }
                            else {
                                rc.writeSharedArray(GlobalArray.OPPONENT_HEADQUARTERS, (opponentHeadquarters & 0b100) | (headquarterID + 1));
                            }
                        }
                        noRobotTime = 0;
                    }
                ...
                indicatorString.append("RBT TIME " + noRobotTime + "; ");
                return;
            }
